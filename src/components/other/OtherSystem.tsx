...
import {
  ExternalLink, Plus, Search, Trash2, Edit3, X, Link as LinkIcon,
  Map, FileText, BookOpen, Grid,
} from 'lucide-react';
...
const BruneiMap = dynamic(() => import('./BruneiMap'), { ssr: false });

type Tab = 'manuals' | 'publications' | 'links' | 'map' | 'applications';

interface OtherSystemProps {
  isAdmin: boolean;
}

const emptyForm: SharedLinkFormData = { title: '', url: '', comment: '' };

const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
  { id: 'manuals', label: 'Manuals', icon: <FileText className="w-4 h-4" /> },
  { id: 'publications', label: 'Publications', icon: <BookOpen className="w-4 h-4" /> },
  { id: 'applications', label: 'Applications', icon: <Grid className="w-4 h-4" /> },
  { id: 'links', label: 'Links', icon: <ExternalLink className="w-4 h-4" /> },
  { id: 'map', label: 'Brunei Map', icon: <Map className="w-4 h-4" /> },
];

export default function OtherSystem({ isAdmin }: OtherSystemProps) {
  const [activeTab, setActiveTab] = useState<Tab>('manuals');

  // Lazy-mount flags — each heavy tab mounts once and stays mounted
  const [manualsOpened, setManualsOpened] = useState(true); // default tab, open immediately
  const [pubsOpened, setPubsOpened] = useState(false);
  const [mapOpened, setMapOpened] = useState(false);

  function switchTab(tab: Tab) {
    setActiveTab(tab);
    if (tab === 'manuals') setManualsOpened(true);
    if (tab === 'publications') setPubsOpened(true);
  ...
  // Fetch links only when that tab is first opened
  useEffect(() => {
    if (activeTab === 'links' && !linksLoaded) fetchLinks();
  }, [activeTab, linksLoaded, fetchLinks]);
  ...
  const startEdit = (link: SharedLink) => {
    setEditingLink(link);
    setFormData({ title: link.title, url: link.url, comment: link.comment || '' });
    setShowAddForm(false);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Manuals tab */}
      <div className={activeTab === 'manuals' ? 'block' : 'hidden'}>
        {manualsOpened && <ManualsSystem isAdmin={isAdmin} />}
      </div>

      {/* Publications tab */}
      <div className={activeTab === 'publications' ? 'block' : 'hidden'}>
        {pubsOpened && <PublicationsSystem isAdmin={isAdmin} />}
      </div>
      ...
      <div className={activeTab === 'map' ? 'block' : 'hidden'}>
        {mapOpened && <BruneiMap isAdmin={isAdmin} />}
      </div>

      {/* Applications tab */}
      <div className={activeTab === 'applications' ? 'block' : 'hidden'}>
        <div className="text-center py-16 text-gray-400">
          <Grid className="w-14 h-14 mx-auto mb-4 opacity-40" />
          <p className="text-xl font-medium text-gray-500">Applications</p>
          <p className="mt-2 text-sm">Links to lab applications will appear here.</p>
        </div>
      </div>
    </div>
  );
